...
document.getElementById("myForm").onsubmit = function(e) {
    ...
    let formSubmission = function() {
        ...
        if (userName === '' && userHeading === '') { hr.remove(); }
        if ((userName === '' || userHeading === '') && (userPic !== '')) { hr.remove(); }
...
document.getElementById("dwn-btn").addEventListener("click", function() {
    ...
    let filteredText2 = filteredText.replace(/<script src=\"https:\/\/www.linkedin.com\/([\s\S]*?)<\/head>/g, '</head>');
    let filteredText3 = filteredText2.replace(/<div class="hiddendiv common">([\s\S]*?)<\/html>/g, '<\/body><\/html>');
